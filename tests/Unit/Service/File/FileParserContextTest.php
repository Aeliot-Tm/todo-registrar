...
final class FileParserContextTest extends TestCase
{
    ...
    private function getExpectedContexts(): array
    {
        ...
        return [
            ...
            17 => "File: {$filePath} -> Namespace: App\Models -> Class: Product -> Method: getPrice -> Match",
            ...
            27 => "File: {$filePath} -> Namespace: App\Enums -> Enum: Status -> Enum_case: Active",
            ...
            42 => "File: {$filePath} -> Namespace: App\Controller -> Class: Entity -> Class_const: MAX_SIZE",
            43 => "File: {$filePath} -> Namespace: App\Controller -> Class: Entity -> Class_const: MIN_SIZE, DEFAULT_SIZE",
    ...
    private function buildContextPath(array $nodes): string
    {
        ...
        foreach ($nodes as $node) {
            $parts[] = match ($node->getKind()) {
                ...
                ContextNodeInterface::KIND_MATCH => 'Match',
