...
Configuration file can either in YAML format ([see documentation about config YAML-file](global_config_yaml.md))
or PHP format ([see documentation about config PHP-file](global_config_php.md)). You may define custom path
...
Otherwise, you may pass a special value`--config=STDIN` then [it obtains YAML from STDIN](global_config_yaml.md#loading-from-stdin).
